...
//		return (short)(((tmp[1]&0xFF)<<8) | (tmp[0]&0xFF));
    ...
    static public String getMidStr(String src, String patLeft, String patRight) {
        int patLen = 0;
        int leftPos = 0, rightPos = 0;
        if (src == null || patLeft == null || patRight == null || src.length() == 0
                || (patLen = patLeft.length()) == 0 || patRight.length() == 0)
            return null;
        
        leftPos = src.indexOf(patLeft);
        if (leftPos >= 0) {
            rightPos = src.indexOf(patRight, leftPos + patLen);
            if (rightPos >= 0) {
                return src.substring(leftPos + patLen, rightPos);
            }
        }
        return null;
    }
